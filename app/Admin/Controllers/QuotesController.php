...
class QuotesController extends AdminController
{
    ...
    {
        return Grid::make(Quote::with(['customer', 'customer_contact']), function (Grid $grid) {
            ...
            $grid->column('processing_status')
                ->filter(
                    Grid\Column\Filter\In::make(Quote::$processingStatusMap)
                )
                ->display(function ($processing_status) {
                    return Quote::$processingStatusMap[$processing_status];
                })
                ->label();
